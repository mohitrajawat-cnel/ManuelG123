 ...
 */
class prestapros_apiconnectorCreatecustomorderModuleFrontController extends ModuleFrontController
{
	...
	public function initContent()
	{
		 ...
		 if(isset($pidset) && $pidset !='')
		{
				...
				$link="https://".$keyget."@".$shopdomain."/api/orders?ws_key=".$keyget."&filter[id]=[$pidset]&output_format=JSON";
			
		}
		else
		{
		
			 $link="https://".$keyget."@".$shopdomain."/api/orders?ws_key=".$keyget."&limit=".$start.",".$limit."&sort=[id_DESC]&output_format=JSON";
		}
	...
	public function getproductinformation($pid,$shopdomain,$languageid)
	{
		...
		{
				  $link="https://".$keyget."@".$shopdomain."/api/orders/".$pid."?ws_key=".$keyget."&output_format=JSON";
